...
(function($jsRoot) {
    ...
    addMethods: function(source) {
      ...
      for (var i = 0, length = properties.length; i < length; i++) {
        ...
        if (
          ancestor &&
          isFunction &&
          FunctionUtils.argumentNames(value)[0] === "$super"
        ) {
          var method = value;
          /** @inner */
          value = (function(m) {
  ...
  Object.extendEx = function(destination, source, options) {
    ...
    for (var property in source) {
      ...
      continue;
      if (ops.ignoreEmptyString && value === "") continue;

      var oldValue = destination[property];
      if (options.preserveExisted && oldValue) continue;
      var oldProto =
      oldValue && oldValue.constructor && oldValue.constructor.prototype;
      ...
      else destination[property] = value;
    }
    return destination;
  };
  /** @ignore */
  ...
  Object.getCascadeFieldValue = function(fieldName, root, Kekule) {
    var result;
    var cascadeNames;
    if (fieldName.length && fieldName.splice)
    // is an array
  ...
  /** @ignore */
  Object.setCascadeFieldValue = function(
    fieldName,
    value,
    root,
    forceCreateEssentialObjs
  ) {
    var cascadeNames;
    if (fieldName.length && fieldName.splice)
    // is an array
    ...
    for (var i = 0, l = cascadeNames.length; i < l; ++i) {
      ...
      if (i === l - 1) {
        ...
        var obj = parent[name];
        if (!obj && forceCreateEssentialObjs) {
          // create new obj
          obj = {};
          parent[name] = obj;
        }
  ...
  */
  ...
  Object.copyValues = function(dest, src, propNames) {
    if (!propNames) return Object.extend(dest, src);
    else {
      for (var i = 0, l = propNames.length; i < l; ++i) {
        var prop = propNames[i];
        var value = src[prop];
        if (value !== undefined) dest[prop] = value;
      }
      return dest;
    }
  };
  ...
  Object._extendSupportMethods(Function.prototype, {
    ...
    wrap: function(wrapper) {
      ...
      return function() {
        return wrapper.apply(
          this,
          [__method.bind(this)].concat(__$A__(arguments))
        );
      };
    },
    methodize: function() {
    ...
    defer: function() {
      var __method = this, args = __$A__(arguments);
      args.shift()
      return window.setTimeout(function() {
        return __method.apply(__method, args);
      }, 10);
  ...
  Object._extendSupportMethods(String.prototype, {
    ...
    pad: function(length, padString, rightPad) {
      ...
      var str = this;
      if (rightPad) return str + a.join("");
      else return a.join("") + str;
    },
  ...
  var StringUtils = {
    ...
    */
    serializeValue: function(value, unchangeTypes) {
      ...
      if (utypes.indexOf(vtype) >= 0) return value.toString();

      if (value === null) return StringUtils.SNULL;
      else if (typeof value == "undefined") return StringUtils.SUNDEFINED;
      else if (value != value)
      // NaN
      ...
      switch (str) {
        ...
        default: {
          if (preferedType) {
            ...
            // guess
            var firstChar = str.charAt(0), s;
            switch (firstChar) {
              case StringUtils.SPOSITIVE:
              case StringUtils.SNEGATIVE: { // may be number
                s = str.substring(1);
                if (StringUtils.isNumbericStr(s))
                // really number or number like 1e20
                return parseFloat(str);
                else return str;
              }
  ...
  */
  addProperty: function(propName, options) {
    ...
    if (propInfo.serializable === undefined) propInfo.serializable = true;
  ...
  */
  indexOf: function(propName) {
    for (var i = 0, l = this.props.length; i < l; ++i) {
      if (this.props[i].name === propName) return i;
    }
    return -1;
  },
  /**
  ...
  */
  getPropInfoAt: function(index) {
    if (index >= 0) return this.props[index];
    else return null;
  },
  /**
  ...
  */
  getPropInfo: function(propName) {
    var result;
    var index = this.indexOf(propName);
    result = this.getPropInfoAt(index);
    return result;
  },
  /**
...
var ClassEx = {
  ...
  */
  ...
  } else result = findOwnList(aClass, scopes);
  return result;
},
...
ObjectEx = Class.create(
  ...
  */
  getAllPropList: function() {
    ...
    } else result = this.getOwnPropList().clone();
    return result;
  },
  /**
  * Get list of all properties of certain scopes in this class, including ones inherited from parent class.
  ...
  */
  ...
  createPropGetter: function(prop, getter) {
    ...
    if (!actualGetter) {
      actualGetter =
      getter || new Function('return this["' + prop.storeField + '"];');
...
notifyPropSet: function(propName, newValue, doNotEvokeObjChange) {
  if (this.isUpdating()) {
    // in update state, just queue modified property names
  ...
  if (
    this._isPropGetOrSetEvent(propName) ||
    this.isEventPropName(propName)
  )
  return;
...
isEventHandlerList: function(value) {
  return (
    value &&
    typeof value == "object" &&
    value instanceof Class.EventHandlerList /*value.add && value.clear*/
  );
},
/**
...
endUpdate: function() {
  ...
  if (!this.isUpdating()) {
    // update end, notify changed properties
